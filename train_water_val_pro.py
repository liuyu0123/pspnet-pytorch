...
def parse_args():
    parser = argparse.ArgumentParser(description='PSPNet 水域分割训练（优化版）')
    ...
    # 优化器配置 - 关键修复：确保--lr和--learning-rate完全等价
    parser.add_argument('--lr', '--learning-rate', dest='lr', type=float, default=1e-2,
    ...
    parser.add_argument('--model-name', type=str, default=None,
                        help='模型保存文件名（不含扩展名），例如 experiment1')
    parser.add_argument('--log-name', type=str, default=None,
                        help='日志文件名（不含扩展名），例如 experiment1')
...
if __name__ == "__main__":
    ...
    Init_lr = args.lr  # 使用解析后的lr（兼容--lr和--learning-rate）
    Min_lr = Init_lr * 0.01
    ...
    if args.images is not None and args.masks is not None:
        ...
        if args.val_images is not None and args.val_masks is not None:
            ...
            train_lines = get_files_from_dir(train_image_dir, train_label_dir, 
                                             args.mask_ext, args.mask_suffix)
    ...
    if local_rank == 0:
        ...
        show_config(
            ...
            num_workers=args.workers, num_train=num_train, num_val=num_val
        )
